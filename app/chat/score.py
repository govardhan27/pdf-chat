...
def get_scores():
    ...
    for compoent_type in aggregate.keys():
        values = client.hgetall(f"{compoent_type}_score_values")
        counts = client.hgetall(f"{compoent_type}_score_counts")

        for name in values.keys():
            count = int(counts.get(name, 1))
            score = int(values.get(name, 1))
            avg = score / count
            aggregate[compoent_type][name] = [avg]
